...
} from "@/lib/sourcing/talentScoring";
import {
  sourcingAuditEvents,
  safeStringArray,
...
export async function runInternalTalentSearchForProjectBrief(projectBriefId: string) {
  const { buildProjectUnderstanding } = await import("@/lib/producer/projectUnderstanding");
  const { generateRoleMap } = await import("@/lib/producer/roleMap");
