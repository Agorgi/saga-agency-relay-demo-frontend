...
const adminAppDir = join(root, "src/app/(admin)/admin");
...
async function main() {
  ...
  const navHrefs = new Set(flattenAdminNavItems().map((item) => item.href.split("?")[0]));
  ...
  for (const route of pageRoutes) {
    ...
    assert.ok(
      routeInventoryPaths.has(route) ||
        navHrefs.has(route) ||
        (route.includes("[") &&
          (routeInventoryPaths.has(parentRoute) || navHrefs.has(parentRoute))),
      `${route} missing from route inventory and nav config`,
    );
  }
