...
| Crew generation wired into tracer | Real | PR #50: `generateCrewForProject(projectId)` fires on the first `/projects/[id]/crew` visit, materializing producer-generated `RoleOpening` + `Opportunity` rows plus `CandidateRecommendation` rows scored from the internal `CreatorProfile` pool. Idempotent on existing roles. Today the staging Neon pool is sparse, so the realistic state for a fresh project is "real producer roles, 0 internal candidates" — still a major improvement over the prior "no roles at all" empty state. The OpenAI web research provider (`src/sms-engine/sourcing/openaiWebResearchProvider.ts`) is not yet on this path; layering it on top is the natural next step once LLM mode is stable. |
| Candidate research (public web) | Real, admin-only | `src/sms-engine/sourcing/openaiWebResearchProvider.ts` |
| Outreach drafting | Real | `OutboundDraft` rows (type CANDIDATE_OUTREACH) generated by the producer agent and now surfaced to users via `/projects/[id]/outreach` (PR #34). The legacy `Outreach` table is admin-only and not on the tracer path. |
