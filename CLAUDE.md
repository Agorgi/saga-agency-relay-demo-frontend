...
- **Person identity-graph fields:** `Person.fandoms: String[]` and `Person.interests: String[]` (added PR #63, migration `20260518040000_add_person_fandoms_interests`). Both default to `[]`. GIN-indexed for Postgres array-overlap (`&&`) and contains (`@>`) queries. Captured across every persona — fandoms are specific media/franchises (anime, K-pop, Love and Deepspace); interests are broader preferences (nightlife, brunch, raves). Foundation for cross-pollination: a host who mentions "Love and Deepspace" in their brief surfaces to other Love-and-Deepspace fans. **This lifts the "identity + fandom preferences graph" backlog item** that was previously deferred per the "Deferred backlog" section below — now actively being built across PRs #63–68. PR #64 wires the extractors to populate; PR #68 ships the matching helper.
- **Identity signal extraction:** `src/lib/identitySignals.ts` is the cross-persona pattern bank (`FANDOM_PATTERNS` + `INTEREST_PATTERNS`) plus `extractIdentitySignals(text)` and `mergeIdentitySignals(a, b)`. `src/lib/sessionPersonStore.ts` owns the DB side: `ensureSessionPerson(sessionId)` creates the session's anchor `Person` lazily on the first identity-bearing turn (source=APP, no PII), and `upsertSessionIdentitySignals({ sessionId, message })` extracts + merges + writes. Wired into `POST /api/web-chat` so every turn runs the capture, regardless of persona. Defensive: failures are caught + Sentry-tagged so identity capture never breaks chat. `WebSession.personId` was added in migration `20260518050000_add_web_session_person`. The regex pattern bank now serves as the **safety-net fallback** behind the LLM extraction (PR #65) — when live LLM mode is on, the LLM's `extractedSignals.fandoms` / `.interests` take precedence and the regex only fills gaps the LLM didn't catch.
