...
- **Identity signal extraction:** `src/lib/identitySignals.ts` is the cross-persona pattern bank (`FANDOM_PATTERNS` + `INTEREST_PATTERNS`) plus `extractIdentitySignals(text)` and `mergeIdentitySignals(a, b)`. `src/lib/sessionPersonStore.ts` owns the DB side: `ensureSessionPerson(sessionId)` creates the session's anchor `Person` lazily on the first identity-bearing turn (source=APP, no PII), and `upsertSessionIdentitySignals({ sessionId, message })` extracts + merges + writes. Wired into `POST /api/web-chat` so every turn runs the capture, regardless of persona. Defensive: failures are caught + Sentry-tagged so identity capture never breaks chat. `WebSession.personId` was added in migration `20260518050000_add_web_session_person`. Pattern list intentionally biased toward Saga's design-partner cohort today (anime / K-pop / Love and Deepspace + LA / NYC / Brooklyn venue types); will grow as we observe more input until LLM live mode picks up the extraction.
